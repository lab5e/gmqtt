	...
	for _, v := range tt {
		t.Run(v.name, func(t *testing.T) {
			...
			for _, topic := range v.in.Topics {
				...
				if v.shouldSendRetained {
					retainedDB.EXPECT().GetMatchedMessages(sub.TopicFilter).Return([]*entities.Message{v.retainedMsg})
				}
			}
...
func TestClient_publishHandler_common(t *testing.T) {
	...
	for _, v := range tt {
		t.Run(v.name, func(t *testing.T) {
			...
			c, er := srv.newClient(noopConn{})
			a.NoError(er)
			c.deliverMessage = func(srcClientID string, msg *entities.Message, options subscription.IterationOptions) (matched bool) {
				a.Equal(v.clientID, srcClientID)
				a.Equal(entities.MessageFromPublish(v.in), msg)
		...
		t.Run(v.name, func(t *testing.T) {
			...
			if v.retainedAvailable {
				if len(v.in.Payload) == 0 {
					...
					retainedDB.EXPECT().AddOrReplace(entities.MessageFromPublish(v.in))
				}
			}
			err := c.publishHandler(v.in)
			a.Equal(v.err, err)
...
func TestClient_publishHandler_matchTopicAlias(t *testing.T) {
	...
	var deliveredMsg []*entities.Message
	...
	a.Equal(entities.MessageFromPublish(delivered), deliveredMsg[0])
	a.Equal(entities.MessageFromPublish(delivered), deliveredMsg[1])
}
...
func TestMsg_TotalBytes(t *testing.T) {
	...
	for _, v := range tt {
		t.Run(v.name, func(t *testing.T) {
			...
			err := v.pub.Pack(b)
			a.Nil(err)
			msg := entities.MessageFromPublish(v.pub)
