...
func (client *client) SessionInfo() *entities.Session {
...
func (client *client) publishHandler(pub *packets.Publish) *codes.Error {
	...
	msg = entities.MessageFromPublish(pub)
...
func (client *client) readHandle() {
	...
	for packet := range client.in {
		...
		case *packets.Subscribe:
			codeErr = client.subscribeHandler(p)
		case *packets.Publish:
			codeErr = client.publishHandler(p)
		case *packets.Puback:
			codeErr = client.pubackHandler(p)
		case *packets.Pubrel:
			codeErr = client.pubrelHandler(p)
		case *packets.Pubrec:
			client.pubrecHandler(p)
		case *packets.Pubcomp:
			client.pubcompHandler(p)
		case *packets.Pingreq:
			client.pingreqHandler(p)
...
func (client *client) pollInflights() (cont bool, err error) {
	...
	for _, v := range elems {
		...
		case *queue.Publish:
			...
			client.write(entities.MessageToPublish(m.Message, client.version))
		case *queue.Pubrel:
